 ...
 * 通用arm 姿态估计
 */
public class TestInferPoseTask extends BaseTestTask<Void, CharSequence, CharSequence> {
    private static final int NUM_OF_RUNS = 1;
    private static final int NUM_OF_API_CALLS = 1;

    public TestInferPoseTask(Context context, TextView tv, String serialNum) {
    ...
    protected CharSequence doInBackground(Void... voids) {
        ...
        try {
            for (int i = 0; i < NUM_OF_RUNS; i++) {
                ...
                /* 2.1 准备图片，作为Bitmap输入 */
                InputStream is = context.getAssets().open("test_pose.jpg");
                Bitmap image = BitmapFactory.decodeStream(is);
                is.close();
                pLog("Image size: " + image.getWidth() + "*" + image.getHeight());

                /* 2.2 推理图片及解析结果 */
                List<PoseResultModel> results = null;
                String resStr;
                for (int j = 0; j < NUM_OF_API_CALLS; j++) {
                    // 在模型销毁前可以不断调用。但是不支持多线程
                    results = manager.pose(image);
                    ...
                    if (results != null) {
                        ...
                        if (results.size() > 0) {
                            resStr += "labelIndex:" + results.get(0).getLabelIndex() + ", "
                                    ...
                                    + "points:" + results.get(0).getPoints();
                        }
                        resStr += "}}";
                    } else {
                        resStr = "{}";
