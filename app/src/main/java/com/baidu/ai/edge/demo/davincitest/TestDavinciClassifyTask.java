package com.baidu.ai.edge.demo.davincitest;
 ...
 * 华为达芬奇NPU 分类
 */
public class TestDavinciClassifyTask extends BaseTestTask<Void, CharSequence, CharSequence> {
    ...
    public TestDavinciClassifyTask(Context context, TextView tv, String serialNum) {
        super(context, tv, serialNum);
    }

    @Override
    protected CharSequence doInBackground(Void... voids) {
        publishProgress("\n\nDavinci Classification\n");
        try {
            for (int i = 0; i < NUM_OF_RUNS; i++) {
                ...
                DDKDaVinciConfig config = new DDKDaVinciConfig(context.getAssets(), "davinci");
                DavinciManager manager = new DavinciManager(context, config, serialNum);
                ...
                for (int j = 0; j < NUM_OF_API_CALLS; j++) {
                    ...
                    results = manager.classify(image, CONFIDENCE);
